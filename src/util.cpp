...
const char * const BITCOIN_CONF_FILENAME = "introveriumcoin.conf";
const char * const BITCOIN_PID_FILENAME = "introveriumcoind.pid";
...
#else
    const char* pszModule = "introveriumcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Introveriumcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Introveriumcoin";
#else
    // Unix
    return pathRet / ".introveriumcoin";
#endif
#endif
}
